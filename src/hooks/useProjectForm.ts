...
export const useProjectForm = (project?: Project) => {
  ...
  const [prevProjectMeta, setPrevProjectMeta] = useState({
    id: project?.id,
  ...
  const updateField = <K extends keyof ProjectFormData>(field: K, value: ProjectFormData[K]) => {
    if (field === 'tags' && typeof value === 'string') {
      ...
      setFormData((prev) => ({
        ...prev,
        narrative: { ...prev.narrative, ...(value as Record<string, string>) },
      }));
    } else if (field === 'comparison') {
      setFormData((prev) => ({
        ...prev,
        comparison: { ...prev.comparison, ...(value as Record<string, string>) },
      }));
    } else {
