...
function checkCompressRateLimit(identifier: string): RateLimitResult {
  const now = Date.now();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const safeUnlink = async (p: string) => {
      ...
      try { fs.unlinkSync(p); } catch { /* Silently handle unlink failures */ }
    };
      ...
      try {
        if (ext === '.icns') {
          ...
          try {
            ...
            const sharpMsg = sharpError instanceof Error ? sharpError.message : 'Unknown error';
            throw new Error(`Image compression failed: ${sharpMsg}`);
          }
        }
      } catch (err: unknown) {
