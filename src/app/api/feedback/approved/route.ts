...
export async function GET(request: NextRequest) {
    try {
        ...
        const snap = await db.ref('feedback').orderByChild('createdAtMs').limitToLast(500).once('value');
        ...
        console.error('[API /feedback/approved GET] Error:', error instanceof Error ? error.message : error);
