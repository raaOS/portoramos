...
import { success, badRequest, rateLimit, serverError, unauthorized, validationError } from '@/lib/api-response';
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (clientId && await hasRecentSubmissionFromClient(clientId)) {
            ...
            return success({ received: true, duplicate: true }, 'Kamu sudah kirim feedback belum lama ini');
        }
        ...
        void sendFeedbackNotification(telegramLines.join('\n')).catch((err) => {
            console.warn('[Feedback] Telegram notification failed', err instanceof Error ? err.message : err);
        });
...
export async function GET(request: NextRequest) {
    try {
        ...
        const snap = await db.ref('feedback').orderByChild('createdAtMs').limitToLast(limit).once('value');
