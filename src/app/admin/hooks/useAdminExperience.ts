...
export function useAdminExperience(csrfToken: string | null) {
  ...
  const updateExperience = async (updateData: Partial<ExperienceData>) => {
    try {
      ...
      showError(
        `Failed to update experience: ${errorData.error || response.statusText} (${response.status})`
      );
      return false;
    } catch (error) {
      showError(
        `Failed to update experience: ${error instanceof Error ? error.message : 'Network error'}`
      );
      return false;
    }
  };
