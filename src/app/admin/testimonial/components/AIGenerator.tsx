...
export default function AIGenerator({ onGenerate, isLoading }: AIGeneratorProps) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-green-100 bg-gradient-to-br from-green-50 to-emerald-50 p-6 shadow-sm">
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-col gap-3 md:flex-row">
          <input
            ...
            className="flex-1 rounded-xl border border-green-200 bg-white px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-green-500"
            placeholder="Contoh: Desain Banner Ads / Manipulasi Produk"
          />
          <select
            value={count}
            onChange={(e) => setCount(Number(e.target.value))}
            className="rounded-xl border border-green-200 bg-white px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-green-500 md:w-32"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-xl bg-green-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-green-200 transition-all hover:bg-green-700 disabled:opacity-50"
          >
            {isLoading ? (
