...
export default function StickyNotesManager({ }: StickyNotesManagerProps) {
    ...
    const loadNotes = async () => {
        try {
            ...
            showError('Failed to load sticky notes.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        try {
            ...
            if (response.ok) {
                showSuccess('Sticky notes saved successfully.');
            } else {
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start sm:items-center justify-between gap-4">
                <div>
                    <h3 className="font-bold text-blue-800 flex items-center gap-2">
                        <CheckSquare className="w-5 h-5" /> Sticky Notes Manager
                    </h3>
                    <p className="text-sm text-blue-600 mt-1">
                        Create, edit, or delete sticky notes that appear on the OS desktop.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-6">
                ...
                {notes.length === 0 && (
                    <div className="col-span-full py-12 text-center border-2 border-dashed border-gray-200 rounded-xl bg-gray-50">
                        ...
                        <h3 className="text-gray-500 font-medium">No sticky notes found</h3>
                        <p className="text-gray-400 text-sm mt-1">Click "Add Note" to create your first one.</p>
                    </div>
                )}
            </div>
        </div>
