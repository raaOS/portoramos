...
import { Database, Wifi, Cloud, Activity, Shield, Globe, Bot, Settings, RefreshCw, List, HardDrive, Image as ImageIcon, Film, AlertTriangle, Info, HelpCircle, CloudUpload, CheckCircle2, XCircle, Loader2, X as XIcon } from 'lucide-react';
...
// ─── UploadsPopout ────────────────────────────────────────────────────────
// Detail per-file untuk semua background upload task aktif. Sebelumnya badge
// inline di menubar nampilkan "X/Y · Z%" rata-rata. Sekarang dipindah ke
// popout supaya menubar tetap ringkas dan admin bisa lihat status individual
// (filename, progress per-task, statusDetail dari WASM ffmpeg, error message).
    ...
    <PopoutShell isOpen={isOpen} onClose={onClose} anchorRef={anchorRef} width={340}>
      ...
      <div className="max-h-[360px] space-y-2 overflow-y-auto p-3">
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <UploadTaskRow key={task.id} task={task} onRemove={onRemoveTask} />
          ))
        )}
      </div>
