...
export default function DesktopIcon({ label, icon, imageUrl, videoUrl, onClick, x = 0, y = 0, size = "medium", aspectRatio = 1 }: DesktopIconProps) {
    ...
    return (
        ...
        >
            ...
            <span className="text-xs text-black font-medium text-center px-2 py-1 bg-white/40 rounded-[2px] backdrop-blur-md shadow-sm border border-white/30 opacity-0 group-hover:opacity-100 transition-all duration-200 max-w-[120px] truncate select-none absolute top-full mt-1 z-20">
                {label}
            </span>
        </motion.div>
    );
