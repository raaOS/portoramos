            ...
            <div className="relative w-full h-full p-5 flex flex-col z-10">
                ...
                <div className={`transition-opacity duration-200 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
                    ...
                    <button
                        ...
                        className="absolute top-3 left-3 p-1.5 bg-black/5 hover:bg-black/10 rounded-full text-black/60 hover:text-red-600 transition-colors z-30"
                        title="Delete Note"
                    >
                        <X size={14} />
                    </button>

                    {/* Color Picker (Top Right) - Dark Pill Style */}
                    <div className="absolute top-3 right-3 flex gap-1.5 bg-gray-900/90 p-1.5 rounded-full shadow-sm backdrop-blur-md z-30 scale-90 origin-top-right">
                        {COLORS.map((c) => (
                            <button
                                key={c.name}
                                onClick={(e) => { e.stopPropagation(); setCurrentColor(c.class); }}
                                className={`w-3 h-3 rounded-full border border-white/10 transition-transform hover:scale-125 ${c.class}`}
                                title={c.name}
                            />
                        ))}
                    </div>
