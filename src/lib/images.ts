...
function cloudinaryPosterFromVideo(u: string){
  ...
  try {
    ...
    // Keep resource as video, request jpg thumbnail at start (so_0)
    const withSo = insertCloudinaryTransform(u, 'so_0')
...
export function resolveCover(p: Project): MediaItem {
  const inferredVideo = isVideoLink(p.cover)
  const kind = (inferredVideo ? 'video' : 'image') as MediaItem['kind']
  if (p.cover) {
    if (inferredVideo) {
...
export function resolveGallery(p: Project): MediaItem[] {
  if (p.galleryItems && p.galleryItems.length) {
    return p.galleryItems.map((it) => {
      if (it.kind === 'video'){
  ...
  const basic = (p.gallery || []).map((src) => {
    ...
    return { kind: 'image', src: toImageProxy(img) } as MediaItem
  })
