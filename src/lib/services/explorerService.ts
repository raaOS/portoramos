...
export const explorerService = {
  ...
  async getNodes(parentId: string | null = null): Promise<AnyExplorerNode[]> {
    const normalize = (id: string | null | undefined) =>
      !id || id === 'root' || id === 'null' || id === 'undefined' ? null : id;
  ...
  async createFolder(name: string, parentId: string | null): Promise<ExplorerFolder> {
    ...
    console.log(
      `[ExplorerService] Creating folder: ${name} (${id}) under parent: ${parentId || 'Root'}`
    );
  ...
  async createFile(
    ...
    console.log(
      `[ExplorerService] Creating file: ${fileNode.name} (${id}) under parent: ${fileNode.parentId || 'Root'}`
    );
