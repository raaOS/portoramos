...
export interface DatabaseReferenceLike {
    ...
    transaction(updateFn: (currentValue: any) => any): Promise<{ committed: boolean; snapshot: DatabaseSnapshotLike }>;
}
...
class D1Snapshot implements DatabaseSnapshotLike {
    constructor(private readonly value: unknown, private readonly keyValue: string | null = null) {}
...
class D1Reference implements DatabaseReferenceLike {
    ...
    private applyQuery(value: unknown) {
        ...
        if (orderByChild) {
            ...
            entries = entries.sort(([, left], [, right]) => compareValues(getNested(left, childPath), getNested(right, childPath)));
        }
