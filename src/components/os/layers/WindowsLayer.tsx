...
const WindowItem = React.memo(function WindowItem({
  ...
  const onUpdatePosition = useCallback(
    (x: number, y: number) => updateWindowPosition(w.id, x, y),
    [updateWindowPosition, w.id]
  );
  const onResize = useCallback(
    (width: number, height: number) => handleWindowResize(w.id, width, height),
    [handleWindowResize, w.id]
  );
  const onResizeEnd = useCallback(
    (width: number, height: number) => handleWindowResizeEnd(w.id, width, height),
    [handleWindowResizeEnd, w.id]
  );
  const onTogglePin = useMemo(
    () => (isAdmin ? () => togglePin(w.id) : undefined),
    [isAdmin, togglePin, w.id]
  );
...
export default function WindowsLayer({ isAdmin, isReady = true }: WindowsLayerProps) {
  ...
  return (
    <m.div
      className="pointer-events-none absolute inset-0 h-full w-full pb-20"
