    ...
    if (loading) {
        return (
            <div className="w-full space-y-3">
                {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="w-full h-20 rounded-2xl bg-white/5 border border-white/10 animate-pulse" />
                ))}
            </div>
        );
    }
                        ...
                        return (
                            <motion.div
                                ...
                                className={`
                                    ...
                                    ${isOpen ? 'bg-black border-white' : 'bg-white/5 border-white/10 hover:border-white/30'}
                                `}
                            >
                                {/* Header / Trigger */}
                                <button
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`
                                            ...
                                            ${isOpen ? 'bg-white/10' : 'bg-white/5 border border-white/10'}
                                        `}>
                                            <Image
                                                src={skill.icon}
                                                alt={skill.name}
                                        ...
                                        <div>
                                            <h3 className={`font-bold text-base md:text-lg ${isOpen ? 'text-white' : 'text-gray-300'}`}>
                                                {skill.name}
                                            </h3>
                                    ...
                                    <motion.div
                                        ...
                                        className={`
                                            ...
                                            ${isOpen ? 'bg-white text-black' : 'bg-white/10 text-gray-400'}
                                        `}
                                    >
                                            ...
                                            <div className="px-4 pb-5 pt-0 pl-[5.5rem]">
                                                ...
                                                <ul className="space-y-2">
                                                    {skill.details.map((detail, idx) => (
                                                        <li
                                                            key={idx}
                                                            className="flex items-start text-sm text-gray-300 animate-fadeIn"
                                                            style={{ animationDelay: `${idx * 50}ms`, opacity: 0, animationFillMode: 'forwards' }}
                                                        >
                                                            <span className="w-1.5 h-1.5 rounded-full bg-white mt-1.5 mr-2.5 flex-shrink-0" />
                                                            {detail}
                                                        </li>
                                                    ))}
                                                </ul>
            ...
            {skills.length > 4 && (
                <div className="flex justify-center pt-2">
                    <button
                        ...
                        className="group flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-white text-white hover:text-black rounded-full transition-all duration-300 font-medium text-sm border border-white/10"
                    >
