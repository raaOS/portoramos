...
const PhotoItem = memo(({
  ...
  const { transform, zIndex, isVisible, imageSize } = useMemo(() => {
    ...
    const size = { width: baseSize, height: baseSize };
    ...
    if (relativePosition < 0) {
      ...
      transformStr = 'rotateY(0deg) translateZ(140px)';
      z = 42;
    } else {
  ...
  }, [index, currentIndex, totalPhotos]);
  ...
  return (
    <motion.div
      className="absolute cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-gray-900 rounded-lg overflow-visible"
      style={{
        left: '50%',
        top: '50%',
    ...
    >
      ...
      >
        {/* Video Element for Hero Videos */}
        {photo.type === 'video' && photo.isHero ? (
          <video
            src={photo.src}
            poster={photo.poster}
...
export default function CoverFlowGallery({
  ...
  const openPhotoModal = useCallback((photo: Photo) => {
    ...
    setIsModalOpen(true);
    onPhotoSelect?.(photo);
  }, [onPhotoSelect, setIsModalOpen, hideNavbar]);
    ...
    >
      ...
      {showControls && (
        <>
          ...
          {/* Sound Control - Outside Navigation, Top Right */}
          ...
          <div className="absolute bottom-[-40px] left-1/2 -translate-x-1/2 flex items-center gap-6 z-10">
            {/* Navigation Controls */}
            <div className="flex items-center justify-between bg-white rounded-full px-4 py-2 border border-gray-200 w-auto">
              {/* Left Side - Previous Button */}
              <button
                onClick={flowLeft}
                className="p-2 rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400 text-gray-500 hover:text-gray-900 hover:bg-gray-100"
