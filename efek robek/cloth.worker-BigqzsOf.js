(function () {
  ...
  const H = (async () => {
      const r = new URL('../cloth-sim.wasm', self.location.href).toString(),
        e = await fetch(r);
      if (!e.ok) throw new Error(`Failed to fetch cloth-sim.wasm: ${e.status}`);
      if (WebAssembly.compileStreaming) return WebAssembly.compileStreaming(e);
  ...
  H.then(() => {
    J = !0;
    for (const { id: r, msg: e } of D) K(r, e);
    D.length = 0;
  }).catch((r) => {
  ...
  function Z(r) {
    ...
    for (let s = 0; s < i; s++) {
      ...
      if (S > 1e-6) {
        const d = 1 / S;
        ((o[n] = _ * d), (o[n + 1] = b * d), (o[n + 2] = k * d));
      } else ((o[n] = 0), (o[n + 1] = 0), (o[n + 2] = 1));
    }
    return l;
  }
  self.onmessage = (r) => {
  ...
  function K(r, e) {
    if (e.type === 'init') {
      (e.debug && (T = !0),
        Y(e.width, e.height, e.segX, e.segY, e.tearThreshold, e.stiffness)
          .then((a) => {
            (P.set(r, a),
              self.postMessage({
                type: 'ready',
            ...
            for (const i of a.pendingMessages) K(r, i);
      ...
      switch (e.type) {
        ...
        case 'releaseGrab':
          typeof e.slot == 'number'
            ? t.exports.release_grab(e.slot)
            : t.exports.release_all_grabs();
          break;
        case 'cut':
        ...
        case 'step': {
          if (T) {
            ...
            const s = performance.now() - l,
              n = t._stats || (t._stats = { sum: 0, max: 0, n: 0 });
            ((n.sum += s),
              s > n.max && (n.max = s),
              n.n++,
              ...
              n.n >= 60 &&
                (console.log(
                  `[cloth.worker id=${r}] step avg=${(n.sum / n.n).toFixed(2)}ms  max=${n.max.toFixed(2)}ms  over ${n.n} steps`
                ),
                (n.sum = 0),
                (n.max = 0),
                (n.n = 0)));
          } else t.exports.step(e.gx, e.gy, e.gz, e.damping, e.iterations, e.subSteps);
          let a = t.posView;
          if (a.buffer !== t.exports.memory.buffer || a.length !== t.exports.pos_len()) {
            const l = t.exports.pos_ptr(),
              s = t.exports.pos_len();
            ((a = new Float32Array(t.exports.memory.buffer, l, s)), (t.posView = a));
          }
          let i;
          const y = e.buffer;
          (y instanceof ArrayBuffer && y.byteLength === a.byteLength
            ? (i = y)
            : (i = new ArrayBuffer(a.byteLength)),
