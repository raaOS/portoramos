...
async function run() {
  ...
  const missingVideoUrl =
    'https://res.cloudinary.com/dcb3dslfw/video/upload/v1766857273/AQMRJH0BtqJVJdw_PX1iLNUZuM7OjhI8KOt8dAx5yn6ryPif1yfQ7wrl9s7ZaP3J-J9oYFbOy8QKyVRA8dUAtzm738de-P5C_yuiotv.mp4';
  ...
  projectsData.projects = projectsData.projects.filter(
    (p) => !p.slug.toLowerCase().includes('synctest')
  );

  // Fix gallery item
  const sneakerProject = projectsData.projects.find(
    (p) => p.slug === 'tempat-perlindungan-sneaker-surealis'
  );
  if (sneakerProject) {
