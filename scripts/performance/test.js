...
class PerformanceTester {
  ...
  testBuildSize() {
    ...
    this.addTest('build_size', passed, `Total build size: ${sizeMB.toFixed(2)}MB ${passed ? '(✅ < 5MB)' : '(❌ > 5MB)'}`);
  }
  ...
  testCriticalCSS() {
    ...
    this.addTest('critical_css', passed, `Critical CSS size: ${sizeKB.toFixed(2)}KB ${passed ? '(✅ < 50KB)' : '(❌ > 50KB)'}`);
  }
  ...
  testImageOptimization() {
    ...
    const imageFiles = this.findFiles(publicDir, ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.avif']);
    ...
    this.addTest('image_optimization', passed, `Found ${largeImages} images > 200KB ${passed ? '(✅ all optimized)' : '(❌ needs optimization)'}`);
  }
  ...
  testCompression() {
    ...
    if (fs.existsSync(nextConfigPath)) {
      ...
      this.addTest('compression', hasCompression, hasCompression ? 'Compression enabled' : 'Compression not configured');
    } else {
  ...
  testCaching() {
    ...
    if (fs.existsSync(nextConfigPath)) {
      ...
      this.addTest('caching', hasCaching, hasCaching ? 'Caching headers configured' : 'Caching not configured');
    } else {
  ...
  testBundleSplitting() {
    ...
    this.addTest('bundle_splitting', passed, `Found ${chunks.length} large chunks (>100KB) ${passed ? '(✅ reasonable)' : '(❌ too many)'}`);
  }
