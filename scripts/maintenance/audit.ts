...
async function checkEnvironment(): Promise<CheckResult> {
    const missing = [
        ...
        process.env.NEXT_PUBLIC_DATA_BACKEND !== 'cloudflare-d1' && 'NEXT_PUBLIC_DATA_BACKEND=cloudflare-d1',
    ].filter(Boolean) as string[];

    return missing.length > 0
        ? { name: 'Environment', status: 'FAIL', message: `Missing or invalid: ${missing.join(', ')}` }
        : { name: 'Environment', status: 'PASS', message: 'Required Cloudflare and app env vars are set' };
}
...
async function checkDataServices(): Promise<CheckResult> {
    ...
    if (!isD1Configured()) {
        return { name: 'DataServices', status: 'FAIL', message: `D1 missing: ${getMissingD1EnvKeys().join(', ')}` };
    }

    if (!isR2StorageConfigured()) {
        return { name: 'DataServices', status: 'FAIL', message: `R2 missing: ${getMissingR2EnvKeys().join(', ')}` };
    }

    const rows = await queryD1<{ count: number }>('SELECT COUNT(*) as count FROM app_kv');
    return { name: 'DataServices', status: 'PASS', message: `D1 reachable, app_kv rows=${rows[0]?.count ?? 0}; R2 configured` };
}
