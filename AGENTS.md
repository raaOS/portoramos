...
|---|---|---|---|
| 1 | `src/components/os/core/Dock.tsx` | Menu Dock bawah; hover scaling fisika ikon, bounce animation, context menus, SoundManager, drag-and-drop `@dnd-kit/core`. | Merusak tracking kursor real-time, mengganggu timing bounce, memicu *infinite re-renders*. |
...
* `db.ref(path)` memparse pemisah `/` sebagai objek bersarang (*nested object*) dalam baris tabel `content`.
...
1. **Direct-to-R2 (Default):** Melalui `BackgroundUploadContext`. Berkas video berukuran besar dikompresi langsung di sisi client menggunakan WASM FFmpeg sebelum diunggah langsung ke Cloudflare R2 menggunakan *presigned URL* untuk memintas batas payload Vercel (4.5 MB).
...
* **GOP (Group of Pictures):** Ditetapkan ke `60` secara eksplisit guna menghindari efek macroblocking saat video wallpaper diputar berulang (*looping*).
